...
class GasProps(ttk.Frame):
    ...
    def init_gui(self):
        ...
        self.T_entry.grid(column=0, row=11)
        self.P_entry.grid(column=0, row=12)
        self.H_entry.grid(column=0, row=13)
        self.S_entry.grid(column=0, row=14)
        self.D_entry.grid(column=0, row=15)
        self.U_entry.grid(column=0, row=16)
        
        ttk.Label(self, text='Initial Conditions').grid(column=0, row=10)
#        ttk.Label(self, text='Temperature').grid(column=0, row=11)
#        ttk.Label(self, text='Pressure').grid(column=0, row=12)
#        ttk.Label(self, text='Enthalpy').grid(column=0, row=13)
#        ttk.Label(self, text='Entropy').grid(column=0, row=14)
#        ttk.Label(self, text='Density').grid(column=0, row=15)
#        ttk.Label(self, text='Energy').grid(column=0, row=16)
        
        self.initial_button = ttk.Button(
            self, text='Calc. Initial Conditions', 
            command=self.calculate_initial)
        self.initial_button.grid(column=0, row=17)
        ...
        self.T_f_entry.grid(column=2, row=11)
        self.P_f_entry.grid(column=2, row=12)
        self.H_f_entry.grid(column=2, row=13)
        self.S_f_entry.grid(column=2, row=14)
        self.D_f_entry.grid(column=2, row=15)
        self.U_f_entry.grid(column=2, row=16)
        
        ttk.Label(self, text='Final Conditions').grid(column=2, row=10)
        ttk.Label(self, text='Temperature (K)').grid(column=1, row=11)
        ttk.Label(self, text='Pressure (Pa)').grid(column=1, row=12)
        ttk.Label(self, text='Enthalpy (J/kg)').grid(column=1, row=13)
        ttk.Label(self, text='Entropy (J/kg/K)').grid(column=1, row=14)
        ttk.Label(self, text='Density (kg/m^3)').grid(column=1, row=15)
        ttk.Label(self, text='Energy (J/kg)').grid(column=1, row=16)
        
        self.final_button = ttk.Button(
            self, text='Calc. Final Conditions', 
            command=self.calculate_final)
        self.final_button.grid(column=2, row=17)
        ...
        ttk.Radiobutton(self, text='Chemically Frozen', variable=self.problem_type, 
                        value=0).grid(column=4, row=0)
        ttk.Radiobutton(self, text='Isentropic Compression. cr:', 
                        variable=self.problem_type, value=1).grid(column=4, 
                                                                  row=1)
        ttk.Radiobutton(self, text='Chemical Equilibrium', variable=self.problem_type, 
                        value=2).grid(column=4, row=2)
        ...
        # Final Composition
        ttk.Label(self, text='Final Composition').grid(column=4, row=10, 
                                                       columnspan=2)
        ttk.Label(self, text='Species').grid(column=4, row=11)
        ttk.Label(self, text='Mole Fraction').grid(column=5, row=11)
        final_spec = []
        final_mole_frac = []
        for ind in range(12, 25):
    ...
    def calculate_initial(self):
        ...
        if len([x for x in outputs if x is not None]) == 2:
            if T is not None and P is not None:
                ...
                tkinter.messagebox.showerror('Error', 'Must input exactly two state variables (TP, HP, SP, SD, UV)')
                return
        elif self.initial_type is None:
            tkinter.messagebox.showerror('Error', 'Must input exactly two state variables (TP, HP, SP, SD, UD)')
            return
    ...
    def calculate_final(self):
        ...
        if self.problem_type.get() == 1:  # Isentropic Compression
            ...
            try:
                ...
                tkinter.messagebox.showerror('Error', 'compression ratio must be a number')
                return
        ...
        try:
            ...
            S = gas.s
            U = gas.u
            [x.set(self.fmt.format(y)) for x, y in zip(inputs, [T, P, H, S, D, U])]
    ...
    def change_phi(self, *args):
        ...
        try:
            ...
            tkinter.messagebox.showerror('Error', 'Must calculate initial conditions before altering equivalence ratio')
            return
